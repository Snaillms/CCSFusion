...
class Evaluator():
    ...
    def MI(cls, image_F, image_A, image_B):
        ...
        # 方法1：离散化处理（将像素值分箱）
        def discretize(img, bins=256):
            # 确保值域在0-255之间
            min_val = np.min(img)
            max_val = np.max(img)
            if max_val > min_val:
                img_scaled = (img - min_val) / (max_val - min_val) * 255
            else:
        ...
        # 方法2：基于直方图的互信息计算（更专业的图像MI计算方法）
        def mutual_information_histogram(image1, image2, bins=256):
            """计算基于直方图的互信息"""
            # 确保图像在0-255范围内
            image1 = discretize(image1)
            image2 = discretize(image2)
            ...
            # 计算边缘概率分布
            pA = hist_1 / float(np.sum(hist_1))
            pB = hist_2 / float(np.sum(hist_2))
            ...
            for i in range(bins):
                for j in range(bins):
                    if pAB[i, j] > 0 and pA[i] > 0 and pB[j] > 0:
                        MI += pAB[i, j] * np.log2(pAB[i, j] / (pA[i] * pB[j]))
            
            return MI
        
        # 使用方法1：离散化后使用sklearn的mutual_info_score
        image_F_disc = discretize(image_F.flatten())
        image_A_disc = discretize(image_A.flatten())
        image_B_disc = discretize(image_B.flatten())
        
        mi_fa = skm.mutual_info_score(image_F_disc, image_A_disc)
        mi_fb = skm.mutual_info_score(image_F_disc, image_B_disc)
        
        # 如果想使用方法2，取消下面的注释
        # mi_fa = mutual_information_histogram(image_F.flatten(), image_A.flatten())
        # mi_fb = mutual_information_histogram(image_F.flatten(), image_B.flatten())
    ...
    def SSIM(cls, image_F, image_A, image_B):
        cls.input_check(image_F, image_A, image_B)
        # 假设 image_A 和 image_B 的数据范围与 image_F 相同 (0-255)
        ssim_FA = ssim(image_F, image_A, data_range=image_F.max() - image_F.min()) # 或者直接写 data_range=255.0 如果确定是这个范围
        ssim_FB = ssim(image_F, image_B, data_range=image_F.max() - image_F.min()) # 或者直接写 data_range=255.0
        return (ssim_FA + ssim_FB) / 2.0 # 通常取平均值
    
    @classmethod
    def MS_SSIM(cls, image_F, image_A, image_B, weights=None, levels=5, win_size=11, sigma=1.5):
        if weights is None:
            weights = np.array([0.0448, 0.2856, 0.3001, 0.2363, 0.1333])
        assert len(weights) == levels, "Weights length must match number of scales"

        def ms_ssim_single(img1, img2):
            # 验证输入数据
            if np.any(np.isnan(img1)) or np.any(np.isnan(img2)):
                return 0.0
            if np.any(np.isinf(img1)) or np.any(np.isinf(img2)):
                return 0.0
            ...
            for _ in range(levels):
                try:
                    ...
                    if np.isnan(score) or np.isinf(score):
                        score = 0.0
                    mssim_vals.append(score)
                except Exception as e:
                    print(f"Warning: Error in MS-SSIM calculation: {e}")
                    mssim_vals.append(0.0)
                    
                img1 = img1[::2, ::2] if img1.ndim == 2 else img1[::2, ::2, :]
                img2 = img2[::2, ::2] if img2.ndim == 2 else img2[::2, ::2, :]
                
            mssim_vals = np.array(mssim_vals)
            # 确保没有无效值
            mssim_vals = np.clip(mssim_vals, 0.0, 1.0)
            return np.prod(mssim_vals ** weights)

        try:
            return (ms_ssim_single(image_F, image_A) + ms_ssim_single(image_F, image_B))/2
        except Exception as e:
            print(f"Warning: Error in final MS-SSIM calculation: {e}")
            return 0.0
